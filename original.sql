...
SELECT from_date as original, DATE_FORMAT(from_date, "%M %d %Y") as new FROM salaries; 

SELECT first_name, last_name , concat(LEFT(first_name, 1), LEFT(last_name, 1)) as Initials from employees;
select concat(LEFT(first_name, 1), LEFT(last_name, 2)) as Initials from employees;
